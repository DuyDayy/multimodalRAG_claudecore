...
from src.agents.retriever_agent import embedder
...
try:
    # embedder has already been instantiated inside retriever_agent
...
import re
def display_claude_response(text: str):
    """Parse thẻ <think> và <answer> để hiển thị UI đẹp hơn."""
    think_match = re.search(r'<think>(.*?)</think>', text, flags=re.DOTALL)
    answer_match = re.search(r'<answer>(.*?)</answer>', text, flags=re.DOTALL)
    
    if think_match:
        with st.expander("🧠 Quá trình tư duy của AI (Chain-of-Thought)"):
            st.markdown(think_match.group(1).strip())
            
    if answer_match:
        st.markdown(answer_match.group(1).strip())
    else:
        # Fallback nếu Claude quên dùng thẻ
...
with tab1:
    ...
    if uploaded_file is not None:
        ...
        if st.button("Tìm kiếm Video Gốc"):
            ...
            st.success("Kết quả:")
            display_claude_response(result["draft_answer"])
            
            context = result.get("retrieved_context", [])
            if context and result.get("strict_keyframe_match"):
                st.info("🎯 **ĐÁP ÁN GROUND TRUTH DUY NHẤT (LUẬT KIS):**")
                payload = context[0].get("payload", {})
                st.write(f"**Video ID:** `{payload.get('video_id')}`")
                st.write(f"**Keyframe Timestamp:** `{payload.get('formatted_time')}` ({payload.get('timestamp_sec')}s)")
                if payload.get('frame_path') and os.path.exists(payload['frame_path']):
                    st.image(payload['frame_path'], width=400)
            st.json(context)
